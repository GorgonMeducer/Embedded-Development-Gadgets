...
//using System.Windows.Forms; // Nedded for Message in DeviceNameMatch()
...
namespace HID
{
	public static class DeviceManagement
	{
		...
		internal static Boolean DeviceNameMatch(IntPtr newDevicePathName, String mydevicePathName)
		{
			Int32 stringSize = 0;

			try
			{
                ESnail.Utilities.Win32API.DBT.DEV_BROADCAST_DEVICEINTERFACE_1 devBroadcastDeviceInterface = new ESnail.Utilities.Win32API.DBT.DEV_BROADCAST_DEVICEINTERFACE_1();
                ESnail.Utilities.Win32API.DBT.DEV_BROADCAST_HDR devBroadcastHeader = new ESnail.Utilities.Win32API.DBT.DEV_BROADCAST_HDR();

				// The LParam parameter of Message is a pointer to a DEV_BROADCAST_HDR structure.

				//Marshal.PtrToStructure(m.LParam, devBroadcastHeader);
                Marshal.PtrToStructure(newDevicePathName, devBroadcastHeader);

                if ((devBroadcastHeader.dbch_devicetype == ESnail.Utilities.Win32API.DBT.DBT_DEVTYP_DEVICEINTERFACE))
				{
                    ...
                    Marshal.PtrToStructure(newDevicePathName, devBroadcastDeviceInterface);
        ...
        public static String GetDevicePath(short vendorID, short productID )
        {
            String  devicePath  = "";
            Guid    mHidGuid    = Guid.Empty;
            ...
            if (devicesFound)
            {
                    ...
                    if (!mHidHandle.IsInvalid)
                    {
                        ...
                        if (success)
                        {
                            if ((mHid.DeviceAttributes.VendorID  == vendorID ) &&
                                (mHid.DeviceAttributes.ProductID == productID)     )
                            {
                                devicePath  = devPathName[memberIndex];                                
                            }
                        }
                ...
                } while (!((devicePath.Length > 0 || (memberIndex == devPathName.Length))));
		...
		///  </returns>
		static internal Boolean FindDeviceFromGuid(System.Guid myGuid, ref String[] devicePathName)
		{
			Int32   bufferSize       = 0;
            ...
            ESnail.Utilities.Win32API.SetupAPI.SP_DEVICE_INTERFACE_DATA MyDeviceInterfaceData = new ESnail.Utilities.Win32API.SetupAPI.SP_DEVICE_INTERFACE_DATA();
			...
			try
			{
				...
				// ***
                deviceInfoSet = ESnail.Utilities.Win32API.SetupAPI.SetupDiGetClassDevs(ref myGuid,
                                                                      IntPtr.Zero,
                                                                      IntPtr.Zero,
                                                                      ESnail.Utilities.Win32API.SetupAPI.DIGCF_PRESENT | ESnail.Utilities.Win32API.SetupAPI.DIGCF_DEVICEINTERFACE);
				...
				do
				{
					...
					//  True on success.
					// ***
                    success = ESnail.Utilities.Win32API.SetupAPI.SetupDiEnumDeviceInterfaces(deviceInfoSet,
						                                                    IntPtr.Zero,
						                                                    ref myGuid,
						                                                    memberIndex,
					...
					else
					{
						...
						//  True on success.
						// ***                     
                        success = ESnail.Utilities.Win32API.SetupAPI.SetupDiGetDeviceInterfaceDetail(deviceInfoSet,
							                                                        ref MyDeviceInterfaceData,
							                                                        IntPtr.Zero,
							                                                        0,
							                                                        ref bufferSize,
			...
			finally
			{
				...
				{
                    ESnail.Utilities.Win32API.SetupAPI.SetupDiDestroyDeviceInfoList(deviceInfoSet);
				}
        ...
        public static Boolean RegisterForHIDDeviceNotifications(IntPtr formHandle, ref IntPtr deviceNotificationHandle)
        {
            deviceNotificationHandle = IntPtr.Zero;

            Guid HID_GUID = Guid.Empty;
            ESnail.Utilities.Win32API.HID.HidD_GetHidGuid(ref HID_GUID);

            return RegisterForDeviceNotifications(formHandle, HID_GUID, ref deviceNotificationHandle);

        }// RegisterForHIDDeviceNotification()
		...
		///  </returns>
		///  
		static internal Boolean RegisterForDeviceNotifications(IntPtr formHandle, Guid classGuid, ref IntPtr deviceNotificationHandle)
		{
			// A DEV_BROADCAST_DEVICEINTERFACE header holds information about the request.
            ESnail.Utilities.Win32API.DBT.DEV_BROADCAST_DEVICEINTERFACE devBroadcastDeviceInterface = new ESnail.Utilities.Win32API.DBT.DEV_BROADCAST_DEVICEINTERFACE();
			...
			try
			{
                ...
                devBroadcastDeviceInterface.dbcc_devicetype = ESnail.Utilities.Win32API.DBT.DBT_DEVTYP_DEVICEINTERFACE;
				...
				//  Returns
				//  ESnail.Device notification handle or NULL on failure.
				// ***
                deviceNotificationHandle = ESnail.Utilities.Win32API.WinUser.RegisterDeviceNotification(formHandle,
                                                                                       devBroadcastDeviceInterfaceBuffer,
                                                                                       ESnail.Utilities.Win32API.WinUser.DEVICE_NOTIFY_WINDOW_HANDLE);
		...
		public static Boolean StopReceivingDeviceNotifications(IntPtr deviceNotificationHandle)
		{
			try
			{
				// ***
				...
				// ***
				//  Ignore failures.
                return ESnail.Utilities.Win32API.WinUser.UnregisterDeviceNotification(deviceNotificationHandle);
			}
			catch
			{
				throw;
        ...
        }// StopReceivingDeviceNotifications()
